...
namespace Minesweeper
{
	public class Mines
	{
		...
		static void Main(string[] args)
		{
			...
			int count = 0;
			bool grum = false;
			List<Scores> shampion4eta = new List<Scores>(6);
			int red = 0;
			int kolona = 0;
			bool flag = true;
			const int maks = 35;
			...
			do
			{
				...
				switch (command)
				{
					...
					case "turn":
						if (bombs[red, kolona] != '*')
						{
							if (bombs[red, kolona] == '-')
							{
								tisinahod(field, bombs, red, kolona);
								count++;
							}
							if (maks == count)
							{
				...
				if (grum)
				{
					...
					Scores t = new Scores(niknejm, count);
					...
					shampion4eta.Sort((Scores r1, Scores r2) => r2.Name.CompareTo(r1.Name));
					shampion4eta.Sort((Scores r1, Scores r2) => r2.Score.CompareTo(r1.Score));
					klasacia(shampion4eta);
				...
				if (flag2)
				{
					...
					Scores to4kii = new Scores(imeee, count);
		...
		private static void klasacia(List<Scores> to4kii)
		{
			Console.WriteLine("\nTo4KI:");
			if (to4kii.Count > 0)
			{
